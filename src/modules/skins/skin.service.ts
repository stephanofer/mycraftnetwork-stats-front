...
export async function getSkinsForPlayers(
  ...
  try {
    const selections = await findSkinSelections(uuids);
    ...
    const urls = selections
      ...
      .map((item) => item.identifier as string);
    const [players, urlValues] = await Promise.all([
      findPlayerTextures([...new Set(playerIds)]),
      findUrlTextures([...new Set(urls)]),
    ]);
